...
import { DEFAULT_REPORT_CODE } from '@/lib/design/mock-data';
import { checkRegistrationCode, type CodeCheck } from './actions';
...
function LoginForm() {
  ...
  // Auth henüz bağlanmadı (PLAN.md Gün 1). Şimdilik hakem ekranına götürür;
  // gerçek akışta giriş sonrası role göre yönlendirilecek.
  function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    router.push(`/review/${DEFAULT_REPORT_CODE}`);
  }
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      <input id="login-email" type="email" placeholder="ad.soyad@ornek.com" className={INPUT} />
      ...
      <div className="mb-[22px] flex justify-end">
        <span className="text-teal cursor-pointer text-[12.5px]">Şifremi unuttum</span>
      </div>

      <button
        type="submit"
        className="bg-ink w-full cursor-pointer border-none p-[14px] font-sans text-[15px] font-semibold text-white"
      >
        Giriş Yap
      </button>
    </form>
...
function RegisterForm() {
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()}>
      ...
      <input id="reg-name" placeholder="Zeynep Demir" className={INPUT} />

      <label className={LABEL} htmlFor="reg-email">
        E-POSTA
      </label>
      <input id="reg-email" type="email" placeholder="ad.soyad@ornek.com" className={INPUT} />
